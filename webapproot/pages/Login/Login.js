...
/*
        ...
        try {
            var rt = hex_sha1(this.passwordInput.getDataValue());
            this.loginErrorMsg.setCaption(rt);
        } catch (err) {
            alert(err);
        }
        */
        var pwEnc = SHA1(this.passwordInput.getDataValue());
        //wm.login([this.usernameInput.getDataValue(), pwEnc], null, dojo.hitch(this, "loginFailed"));

        wm.login([this.usernameInput.getDataValue(), this.passwordInput.getDataValue()], null, dojo.hitch(this, "loginFailed"));
    },

    CountForgetPassword: function(inSender, inData) {
        alert(inSender);
        ...
        numRows = inSender.getCount();
        alert(numRows);
        // step through all rows
        for (i = 0; i < numRows; i++) {

            var aRow = inData.getItem(0);
            alert(aRow.data.PASSWORD);

        }
    },

    loginFailed: function(inResponse) {
        this.loginErrorMsg.setCaption("ผมข้อผิดพลาด ชื่อผู้ใช้งาน หรือ รหัสผ่าน ไม่ถูกต้อง");
        this.usernameInput.focus();

        // Call forget password function
        //this.CountForgetPassword(this.varUser);
    },
 ...
 *  http://www.webtoolkit.info/ < - THANKYOU THANKYOU THANKYOU
 *
 **/
